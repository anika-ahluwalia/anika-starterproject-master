...
/*
This is the class that displays the ink node. It takes in a 
ink node store that contains all the relevant variables of the 
ink node. Then it renders the ink node. It first renders 
...
the node) is the canvas itself. It does this by creating an instance
of the imported CanvasDraw in the render function.
...
export class InkNodeView extends React.Component<IProps> {
    ...
    render() {
        ...
        return (
            <div className="node ink-node" onClick = {this.onClick} style={{ transform: store.Transform}}
    ...
    /*
    This method duplicates the ink node. It creates a new node with all 
    the same variables as the old node and then adds that node to the parent 
    collection.
    */
    duplicate = () => {
        let newNode: InkNodeStore = new InkNodeStore({ X: this.props.store.X - 100, 
            Y: this.props.store.Y - 100, 
            Title: this.props.store.Title, 
            parentCollectionStore: this.props.store.parentCollectionStore,  type: "ink" });
