...
import { Viewer } from "./PDFDoc";
import { observable } from "mobx";

/*
This is the class that displays the PDF node. It takes in a 
PDF node store that contains all the relevant variables of the 
PDF node. Then it renders the PDF node. It first renders 
...
the node) is the PDF itself. It renders the viewer class in this folder.
...
export class PdfNodeView extends React.Component<IProps> {
    ...
    render() {
        ...
        return (
            <div className="node pdf-node" onClick = {this.onClick} style={{ transform: store.Transform}}
            onPointerDown={this.onPointerDown}>
                ...
                <div className="scroll-box">
                    ...
                    <h3 className="title" contentEditable = {true}> {store.Title}</h3>
                         < Viewer url = {this.props.store.Url} />
                    </div>
                </div>
            </div>
        );
    ...
    /*
    This method duplicates the pdf node. It creates a new node with all 
    the same variables as the old node and then adds that node to the parent 
    collection.
    */
    duplicate = () => {
        let newNode: PdfNodeStore = new PdfNodeStore({ X: this.props.store.X - 100, 
            Y: this.props.store.Y - 100, 
            Title: this.props.store.Title, 
            Url: this.props.store.Url, 
            parentCollectionStore: this.props.store.parentCollectionStore,  type: "pdf"  });
